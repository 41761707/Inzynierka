...
\section{Warunki początkowe}
    ...
    Z tego względu wprowadzono nowe pojęcie- \textbf{poziom stanów}, które należy stosować jako oznaczenie wszystkich informacji o świecie w danym momencie.
    \begin{definition}
        \label{PoziomStanow}
    ...
    przy pomocy dwuargumentowej relacji \textit{na}, która jako pierwszy argument przyjmuje sygnaturę robota, a jako drugi- numer kafelka. Na potrzeby przykładu ustalono, iż 
    numerowanie odbywa się rzędami od lewej do prawej. Zgodnie z tymi ustaleniami pozycję robotów A \\
    i B można określić w następujący sposób: \textit{$na(A,4)$} oraz 
    ...
    pustego kafelka. Reasumując, zbiorem stanów początkowy dla analizowanego przykładu \ref{PrzykladSP} jest następujący zbiór: 
    \begin{equation}
...
\section{Akcje}
    \label{RozdzialAkcje}
    Posiadając dobrze określony stan początkowy następnym krokiem będzie zdefiniowanie akcji. Zgodnie z \ref{Akcje} oraz wzmiaką o akcjach w planerze STRIPS,
    akcja musi składać się \\
    ...
    W skład efektów wchodzą dwa pojęcia wprost z terminologii STRIPS- dodające i usuwające. Dzięki takiemu rozróżnieniu łatwiejszym będzie 
    ...
    Następnymi składowymi są odpowiednio \textit{warunki} jak i \textit{efekty}. Warunki traktowane \\
    są jako zbiór wszystkich stanów, które muszą być 
    prawdziwe w danej jednostce czasu. Jeśli choć jeden stan nie jest spełnialny, opisywana akcja nie może zostać wykonana w danym momencie.
    ...
    \begin{figure}[H]
        ...
        \label{PrzykladRuch}
    \end{figure}
    Niech rozpatrywaną akcją będzie przedstawiona na \ref{PrzykladRuch} akcja przemieszczenia robota B z pozycji 3 na pozycję 2. Biorąc pod uwagę, iż stanem początkowym jest \ref{ZbiorPoczatkowy}. warunkami zajścia zdarzenia
    ...
    wykonał ruch z kafelka S na kafelek D, a efektami usuwającymi $\neg na(R,S)$ oraz $\neg pusty(D)$, które informują o tym, iż kafelek S został zwolniony, 
    ...
    Dla przykładu pojęcia $\neg pusty(1)$ oraz $niepusty(1)$ z perpsektywy wprowadzonej terminologii są tożsame. 

        Posiadająć następującą wiedzę poniżej zdefiniowano jedyną akcję znajdująca się w prezentowanym przykładzie:
    \begin{equation}
        \label{Ruch}
        A=(ruch(R,S,D),\{na(R,S),pusty(D)\},\{\{na(R,D),pusty(S)\},\{ \neg na(R,S),\neg pusty(D)\}\})
    \end{equation}
    Podstawiając za $R=B$, $S=3$, a $D=2$ otrzymano następującą akcję:
    \begin{equation}
        A=(ruch(B,3,2),\{na(B,3),pusty(2)\},\{\{na(B,2),pusty(3)\},\{ \neg na(B,3),\neg pusty(2)\}\})
    \end{equation}
    
    Analogicznie można zdefiniować ruch na kafelek numer 6, oraz dwa ruchy dla robota \\
    o sygnaturze A.
    ...
    akcji, zwanych \textbf{akcjami podtrzymującymi}.
    \begin{definition}
        \label{Persist}
        \textbf{Akcja podtrzymująca} -- akcja, która przenosi stan obiektu \\ w czasie $t$ w sposób nienaruszony do poziomu stanów w czasie $t+1$
    \end{definition}
    ...
    \begin{definition}
        \label{Active}
        \textbf{Akcja aktywa} -- akcja, która zmienia stan obiektu między stanami w czasie $t$ oraz $t+1$
    \end{definition}
    ...
    \begin{align*}
        Akcje &= \{zostan(pusty(1)),zostan(pusty(2)),zostan(pusty(5)),zostan(pusty(6)),\\
        &zostan(na(B,3)), zostan(na(A,4)),ruch(B,3,2),ruch(B,3,6),
...
\section{Definiowanie świata}
    ...
    aby mógł bezbłędnie wnioskować \\ 
    w prezentowanej przestrzeni. Analizując przykład \ref{PrzykladSP} dokonano przedstawienia schematu 
    ...
    przemieszczenia. Niespójność ta pojawiła się ze względu na to, iż w żadnym momencie nie określono w jaki sposób dokładnie funkcjonuje czynność oznaczona jako \textit{ruch}.
    ...
    to, iż kafelek drugi sąsiaduje z kafelkiem pierwszym. Wedle tej informacji dla każdej pary sąsiadujących kafelków A i B do zbioru sąsiadów należy dodać dwie relacje:
    $sasiad(A,B)$ oraz $sasiad(B,A)$. Inna sytuacja byłaby, gdyby ruch był możliwy jedynie \\
    w jedną stronę, jednakże tutaj taki stan rzeczy nie występuje.
    ...
    Wymodelowanie tych ograniczeń jest żmudnym, lecz istotnym zadaniem implementując przedstawianą metodologię, które zostanie poruszone \\
    w ramach 
    ...
    przez ten etap, algorytm jest zwarty i gotowy \\
    do generowania planów dla wprowadzonych celów.
...
\section{Warstwy grafu}
    ...
    poziomie akcji. 
    \begin{figure}[H]
        \includegraphics[scale=0.75]{PrzykladRownolegle}
        \centering
    ...
    na porządek liniowy w dowolny sposób. Otóż akcje z danej warstwy mogą być wykonywane w dowolnej kolejności, gdyż w żaden sposób ze sobą nie 
    kolidują. Z persepktywy algorytmu najważniejszym jest, aby stan świata $i$ oraz $i+1$ był zgodny z informacjami jakie posiada o świecie. Zgodnie z
    ...
    O akcjach, jak i stanach, wzajemnie się wykluczających (ang. mutually exclusive, \textbf{mutex}) wspomiano w ramach definiowana świata. Pojęcie to można 
    określić w następujący sposób:
    \begin{definition}
        \label{Warstwa}
        \textbf{Relacją wzajemnie wykluczającą} jest relacją między akcjami(stanami), która informuje o tym, iż nie istnieje plan taki,
    ...
    Przykład dwóch akcji wykluczających przedstawiono \\ 
    na rysunku \ref{PrzykladRownolegle}. Należy zauważyć, iż wzajemne wykluczanie się 
    ...
    i niepusty, co sprowadza się do tego, iż stan $pusty(kafelek)$ jak i $\neg pusty(kafelek)$ są niemożliwe do zawarcia w jednej warstwie planu.
    ...
    \begin{figure}[H]
        ...
        niebieskie między akcjami symbolizują powstałe między nimi \textit{mutex'y}}
        \label{PrzykladWykluczenie3}
    \end{figure}

    Zgodnie z przykładem \ref{PrzykladWykluczenie3} należy zauważyć, iż stany 1 oraz 2 nie mogą 
    znajdować się jednocześnie w planie ze względu na to, iż każda z akcji, która generuje stan 1 (A,B) znajduje się w relacji 
    wzajemnie wykluczającej z każdą akcją, która generuje stan 2 (C,D). Oznacza to, iż wskazane dwa stany powstają w sposób \textbf{niespójny}.
    ...
    \begin{enumerate}
        \item Niespójny efekt -- przypadek, w którym przynajmniej jeden z elementów zbioru efektów pierwszej akcji 
        jest negowany przez przynajmniej jeden z elementów zbioru efektów drugiej
        \item Przeszkadzanie -- przypadek, w którym jedna z akcji usuwa warunki 
        zajścia akcji drugiej 
        \item Konkurencyjne potrzeby -- przypadek, w którym warunki zajścia dwóch akcji 
    ...
    Przyglądająć się powyższej ilustracji mogłoby się wydawać, iż niemożliwym jest wygenerowanie dwóch akcji znajdujących się w relacji wykluczania dla przykładu 
    ...
    $zostan(na(A,lewy))$ jest $na(A,lewy)$, jednakże zbiorem efektów $ruch(A,lewy,prawy)$ jest większy zbiór:
    \begin{equation}
        na(A,prawy), \neg na(A,lewy), pusty(lewy), \neg pusty(prawy)
    \end{equation}
    Widać, iż efekt $\neg na(A,lewy)$ jest negacją efektu $na(A,lewy)$. Akcje podtrzymujące i aktywne względem tego samego obiektu zawsze są ze sobą 
    ...
    musi być możliwy do przedstawienia w postaci porządku liniowego, czyli plan równoległy może zostać przerobiony w dowolny sposób \\
    na plan,
    ...
    jest bardziej skompilowanym zagadnieniem, którego omówienie nastąpi w sekcji odpowiedzialnej \\
    za możliwe rozwinięcia algorytmu.
    ...
    Klocek A do wykonanaia ruchu potrzebuje znajdować się na kafelku środkowym, co oznacza, iż kafelek środkowy musi byc niepusty, 
    jednakże klocek B potrzebuje, aby klocek był pusty. Relacja wykluczająca między tymi stanami bezpośrednio generuje wykluczanie się tych akcji.
    ...
    musi się na nim znajdować, czyli kafelek musi być niepusty. Stany pusty i niepusty względem kafelka\\
    są w oczywistej relacji wykluczającej 
    ...
    z kilku powodów, jednakże algorytm rozpatrując parę akcji podchodzi do tego w sposób binarny, co oznacza, iż patrzy jedynie czy wskazane 
    akcje bądź stany znajdują się w relacji wzajemnie wykluczającej przynajmniej z jednego powodu, nie interesuje go liczba sposobów, na które ów relację można utworzyć.    
...
\section{Wyszukiwanie planu}
    ...
    \begin{enumerate}
        ...
        kombinacji akcji. Jeśli wszystkie dobory akcji zawiodą należy wygenerować kolejną warstwę grafu i ponownie, w warstwie $i+1$, rozpocząć cały proces
        ...
        akcji na ostatnio sprawdzonym poziomie. Brak niepowodzenia na ścieżkę powrotu od warstwy $i$ do warunków początkowych świadczy o tym, 
    ...
    \end{enumerate}
    Aby precyzyjnie przedstawić działanie algorytmu w następnej części pracy przeprowadzono rozbudowaną analizę konkretnego przykładu:
    ...
    $na(A,3)$ znajduje się \\
    w zbiorze stanów poziomu drugiego. Po szybkiej analizie okazuje się, iż cel nie znajduje się na wskazanym poziomie, więc
    należy dokonać rozszerzenia grafu planującego o kolejny poziom. Następuje to w sposób analogiczny.

    \begin{figure}[H]
        \includegraphics[scale=0.3]{FULL_GRAPHPLAN.DEMO1.gv}
        \centering
    ...
    \textbf{StateLevel3}. Następnym krokiem jest sprawdzenie, czy istnieje zbiór akcji, który doprowadzi do stanu $i-1$ bez wprowadzania żadnych sprzeczności.
    ...
    \begin{figure}
        \includegraphics[scale=0.3]{FULL_GRAPHPLAN_DEMO2.gv}
        \centering
        \caption{Graf planujący z zaznaczonymi akcjami aktywnymi prowadzącymi do uzyskania wskazanego celu}
        \label{PrzykladPlanW1}
    \end{figure}

    Rysunek \ref{PrzykladPlanW1} przy pomocy czerwonych strzałek przedstawia akcje aktywną, wraz z jej bezpośrednimi warunkami zajścia oraz efektami, które 
...
\section{Własności GRAPHPLAN'u}
    ...
    znajdują się w utworzonym zbiorze. Jeśli rzeczone cele nie znajdują się w wyznaczonym zbiorze, algorytm wykonuje kolejną iterację, 
    ...
    \begin{proof}
        ...
        Niech warstwa $i$ będzie warstwa, w której nie istnieje możliwość utworzenia planu. W tym przypadku
        dla $i+1$ warstwy algorytm próbuje wygenerować plan cofając się do stanu $i$. Jeśli uda się odpowiednio dobrać akcje 
        cofa się jeszcze dalej aż do stanu początkowego i kończy działanie, jako wynik zwracając plan. Jeżeli dla warstwy $i$ nie udało
        się wygenerować planu,
        a dla warstwy $i+1$ algorytm z powodzeniem utworzył listę kroków do podjęcia, aby wrócić do stanu początkowego, to przez fakt, iż między warstwami 
        $i$ oraz $i+1$ nie znajduje się żadna inna warstwa, można wywnioskować następujący fakt- warstwa $i+1$ jest pierwszą, 
        dla której istnieje możliwość utworzenia planu. Powyższy opis determinuje optymalność planu.
    \end{proof}
    ...
    nowe efekty- dodające bądź usuwające. Mimo iż stan z perspektywy człowieka jest \textbf{usuwający}, 
    z perspektywy algorytmu jest kolejną informacją, która również zostaje dodana do zbioru stanów.

    Korzystając z powyższej własności sformułowano następujący wniosek:
    \begin{theorem}
        \label{LevelOff}
        Liczba stanów w warstwie \textbf{i+1} jest zawsze większa od liczby stanów \\ 
        w warstwie \textbf{i}
    \end{theorem}
    ...
    \begin{proof}
        Zgodnie z \ref{LevelOff} wiadomym jest, iż zawsze spełniona jest zależność \\
        $WSL(i+1) > WSL(i)$. Należy pokazać, iż od pewnego momentu 
        ...
        akcje warunkowe, bądź jakiekolwiek niespodziewane efekty uboczne przeprowadzonych akcji. 
        Algorytm rozpoczyna pracę posiadając odgórnie narzuconą liczbę obiektów, a każdy z obiektów może znajdować się w skończonej liczbie stanów takich 
        ...
        moment krytyczny, dla którego dochodzi do stabilizacji liczby stanów w kreowanych warstwach.
    \end{proof}

    \begin{definition}
        \textbf{Spłaszczeniem} algorytmu jest sytuacja, w której wygenerowanie kolejnego poziomu stanów nie prowadzi 
        do uzyskania nowych stanów w przedstawionym świecie.
    \end{definition}
    ...
    \begin{figure}[H]
        ...
        sygnatury robota jest niemożliwe ze względu na absencję jednego z kafelków (oznaczoną znakiem graficznym \textbf{X}), będących łącznikiem, między robotem a jego celem.}
    \end{figure}
    ...
    aby rzeczony plan zawsze istniał. Jednakże nie zawsze musi tak być, więc należy zabezpieczyć algorytm również \\
    i przed taką ewentualnością. Łatwo zauważyć, 
    ...
    możliwe ruchy nie wprowadzają nowych informacji o świecie. Mimo poczucia, \\
    iż rzeczony mechanizm mógłby poprawnie wykrywać nieskończone generowanie 
    planu, okazuje się, iż porównanie następujących po sobie kolejnych warstw grafu planującego, pod kątem liczby stanów, jest zdecydowanie niewystarczające.
    Z tego powodu wprowadzono dodatkowe pojęcie:
    \begin{definition}
        \textbf{Memoizacja} - mechanizm zapamiętywania stanów nierozwiązywalnych \\
        w danej chwili czasu t.
    \end{definition}

    Podczas próby generowania planu dla złożonych przypadków algorytm często natrafia na ślepe zaułki 
    zanim uda mu się uformować odpowiednią listę kroków. Przy pomocy mechanizmu memoizacji, ów ślepe zaułki zostają 
    zapamiętywane w specjalnej strukturze wraz z warstwą, dla której zaszły. Dzięki powyższemu, po utworzeniu kolejnych warstw 
    i rozpoczęciu procesu cofania, w sytuacji, w której algorytm ponownie natrafi na stan, w którym dokonał procesu memoizacji,
    jest w stanie dokładnie ustalić, które zbiory akcji nie zagwarantują dotarcia do warunków początkowych. Ogranicza to znacznie 
    przestrzeń poszukiwań.
    Memoizacja znacząco wpływa na osiągnięcia czasowe algorytmu. Dodatkowo, możliwość 
    wykrywania nierozwiązywalnych podcelów gwarantuje zatrzymanie algorytmu w sytuacji, gdy 
    utworzenie planu nie jest możliwe (\cite{GRAPHPLAN} sekcja 5.3)
    ...
    Powoduje to, iż GRAPHPLAN jako algorytm jest kompletny i zawsze zakończy swoje działanie w skończonym czasie. Jest to własność, która wyróżnia 
    GRAPHPLAN na tle innych planerów korzystających z częściowego porządku, gdyż z reguły nie posiadają one odpowiednich mechanizmów do wykrywania 
    braku możliwości utworzenia planu.
