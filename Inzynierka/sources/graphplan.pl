...
mutex(P,~P) :- 
    write("Mutex: ["), write(P), write(","),write(~P), writeln("]"),!.   %Negated conditions are always mutually exclusive 
...
mutex(A,B) :-               %Domain-specific pair of inconsistent literals or actions
    ...
    !.   %Defined by the user in planning domain definition
